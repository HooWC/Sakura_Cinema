...
namespace Cinema.API
{
	public class DirectorDB
	{
		...
		public async Task<ObservableCollection<Director>> GetAllDirector_Api()
		{
			client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", JWBToken);
			client.BaseAddress = new Uri(baseUrl);
			var respose = client.GetAsync("Director").Result;

			if (respose.IsSuccessStatusCode)
			{
				string jsonString = respose.Content.ReadAsStringAsync().Result;
				return JsonConvert.DeserializeObject<ObservableCollection<Director>>(jsonString);
			}
		...
		{
			Token_Admin();
			return GetAllDirector_Api().Result;
		}

		public ObservableCollection<Director> GetAllDirector_User()
		{
			Token_User();
			return GetAllDirector_Api().Result;
		}
