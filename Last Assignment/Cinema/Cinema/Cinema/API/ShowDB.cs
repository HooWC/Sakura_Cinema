...
namespace Cinema.API
{
	public class ShowDB
	{
		...
		public async Task<ObservableCollection<Show>> GetAllShow_Api()
		{
			client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", JWBToken);
			client.BaseAddress = new Uri(baseUrl);
			var respose = client.GetAsync("Show").Result;

			if (respose.IsSuccessStatusCode)
			{
				string jsonString = respose.Content.ReadAsStringAsync().Result;
				return JsonConvert.DeserializeObject<ObservableCollection<Show>>(jsonString);
			}

			return null;
		}

		public ObservableCollection<Show> GetAllShowr()
		{
			Token_Admin();
			return GetAllShow_Api().Result;
		}

		public ObservableCollection<Show> GetAllShowr_User()
		{
			Token_User();
			return GetAllShow_Api().Result;
		}
